...
from axe_cli.config import load_config, save_config
from axe_cli.exception import ConfigError
from axe_cli.session import Session
from axe_cli.soul.axesoul import AxeSoul
from axe_cli.ui.shell.console import console
from axe_cli.utils.datetime import format_relative_time
from axe_cli.utils.slashcmd import SlashCommand, SlashCommandRegistry

if TYPE_CHECKING:
    from axe_cli.ui.shell import Shell
...
def _ensure_axe_soul(app: Shell) -> AxeSoul | None:
    if not isinstance(app.soul, AxeSoul):
        console.print("[red]AxeSoul required[/red]")
        return None
    return app.soul
...
def help(app: Shell, args: str):
    ...
    from axe_cli.utils.rich.columns import BulletColumns
...
def version(app: Shell, args: str):
    ...
    from axe_cli.constant import VERSION

    console.print(f"axe, version {VERSION}")


@registry.command
async def model(app: Shell, args: str):
    """Switch LLM model or thinking mode"""
    from axe_cli.llm import derive_model_capabilities

    soul = _ensure_axe_soul(app)
    if soul is None:
        return
    config = soul.runtime.config

    # Managed models refresh removed - using config directly

    if not config.models:
        console.print('[yellow]No models configured. Configure via environment variables or ~/.axe/config.toml[/yellow]')
        return
...
async def clear(app: Shell, args: str):
    """Clear the context"""
    if _ensure_axe_soul(app) is None:
        return
...
async def mcp(app: Shell, args: str):
    ...
    if not isinstance(toolset, AxeToolset):
        console.print("[red]AxeToolset required[/red]")
        return
    ...
    for name, info in servers.items():
        ...
        if info.status == "unauthorized":
            server_text += " [grey50](unauthorized - run: Axe mcp auth {name})[/grey50]"
