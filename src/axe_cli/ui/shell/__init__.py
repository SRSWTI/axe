...
from axe_cli.soul import LLMNotSet, LLMNotSupported, MaxStepsReached, RunCancelled, Soul, run_soul
from axe_cli.soul.axesoul import AxeSoul
from axe_cli.ui.shell.console import console
from axe_cli.ui.shell.prompt import CustomPromptSession, PromptMode, toast
from axe_cli.ui.shell.replay import replay_recent_history
from axe_cli.ui.shell.slash import registry as shell_slash_registry
from axe_cli.ui.shell.slash import shell_mode_registry
from axe_cli.ui.shell.update import LATEST_VERSION_FILE, UpdateResult, do_update, semver_tuple
from axe_cli.ui.shell.visualize import visualize
from axe_cli.utils.envvar import get_env_bool
from axe_cli.utils.logging import open_original_stderr
from axe_cli.utils.signals import install_sigint_handler
from axe_cli.utils.slashcmd import SlashCommand, SlashCommandCall, parse_slash_command_call
from axe_cli.utils.term import ensure_new_line, ensure_tty_sane
from axe_cli.wire.types import ContentPart, StatusUpdate
...
class Shell:
    ...
    async def run(self, command: str | None = None) -> bool:
        ...
        if get_env_bool("AXE_CLI_NO_AUTO_UPDATE"):
            logger.info("Auto-update disabled by AXE_CLI_NO_AUTO_UPDATE environment variable")
        else:
    ...
    async def _run_slash_command(self, command_call: SlashCommandCall) -> None:
        from axe_cli.cli import Reload
    ...
    async def run_soul_command(self, user_input: str | list[ContentPart]) -> bool:
        ...
        try:
            status_snapshot = self.soul.status
            await run_soul(
                self.soul,
                user_input,
                lambda wire: visualize(
                    ...
                    initial_status=StatusUpdate(
                        context_usage=status_snapshot.context_usage,
                        session_input_tokens=status_snapshot.session_input_tokens,
                        session_output_tokens=status_snapshot.session_output_tokens,
                        session_total_tokens=status_snapshot.session_total_tokens,
                    ),
                    cancel_event=cancel_event,
                ),
                cancel_event,
                self.soul.wire_file if isinstance(self.soul, AxeSoul) else None,
            )
            return True
        except LLMNotSet:
            logger.exception("LLM not set:")
            console.print('[red]LLM not set. Configure via environment variables or ~/.axe/config.toml[/red]')
    ...
    async def _auto_update(self) -> None:
        ...
        if result == UpdateResult.UPDATE_AVAILABLE:
            while True:
                toast(
                    "new version found, run `uv tool upgrade axe-cli` to upgrade",
                    topic="update",
                    duration=30.0,
                )
...
def _print_welcome_info(name: str, info_items: list[WelcomeInfoItem]) -> None:
    ...
    if LATEST_VERSION_FILE.exists():
        from axe_cli.constant import VERSION as current_version
        ...
        if semver_tuple(latest_version) > semver_tuple(current_version):
            rows.append(
                Text.from_markup(
                    ...
                    "Please run `uv tool upgrade axe-cli` to upgrade.[/yellow]"
                )
            )
