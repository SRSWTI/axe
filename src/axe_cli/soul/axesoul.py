...
from axe_cli.llm import ModelCapability
from axe_cli.skill import Skill, read_skill_text
from axe_cli.skill.flow import Flow, FlowEdge, FlowNode, parse_choice
from axe_cli.soul import (
    LLMNotSet,
    LLMNotSupported,
    MaxStepsReached,
    Soul,
...
from axe_cli.soul.agent import Agent, Runtime
from axe_cli.soul.compaction import SimpleCompaction
from axe_cli.soul.context import Context
from axe_cli.soul.message import check_message, system, tool_result_to_message
from axe_cli.soul.slash import registry as soul_slash_registry
from axe_cli.soul.toolset import AxeToolset
from axe_cli.token_counter import TokenCounter
from axe_cli.tools.dmail import NAME as SendDMail_NAME
from axe_cli.tools.utils import ToolRejectedError
from axe_cli.utils.logging import logger
from axe_cli.utils.slashcmd import SlashCommand, parse_slash_command_call
from axe_cli.wire.file import WireFile
from axe_cli.wire.types import (
    ApprovalRequest,
    ApprovalResponse,
    CompactionBegin,
    CompactionEnd,
...
class AxeSoul:
    ...
    def __init__(
        ...
        *,
        ...
        model_name = self._runtime.llm.model_name if self._runtime.llm else "gpt-4"
        self._token_counter = TokenCounter(model_name=model_name)
    ...
    def status(self) -> StatusSnapshot:
        token_count = self._token_counter.session_count
        return StatusSnapshot(
            context_usage=self._context_usage,
            yolo_enabled=self._approval.is_yolo(),
            session_input_tokens=token_count.input_tokens,
            session_output_tokens=token_count.output_tokens,
            session_total_tokens=token_count.total_tokens,
        )
    ...
    def token_counter(self) -> TokenCounter:
        """Get the token counter for this soul."""
    ...
    def _make_skill_runner(self, skill: Skill) -> Callable[[AxeSoul, str], None | Awaitable[None]]:
        async def _run_skill(soul: AxeSoul, args: str, *, _skill: Skill = skill) -> None:
    ...
    async def _agent_loop(self) -> TurnOutcome:
        ...
        if isinstance(self._agent.toolset, AxeToolset):
    ...
    async def _step(self) -> StepOutcome | None:
        ...
        async def _kosong_step_with_retry() -> StepResult:
            # Track input tokens before the LLM call
            self._token_counter.reset_step_count()
            self._token_counter.track_input(
                messages=self._context.history,
                system_prompt=self._agent.system_prompt,
                tools=self._agent.toolset.tools if self._agent.toolset else None,
            )
        ...
        # Track output tokens after receiving the response
        self._token_counter.track_output(result.message)
        
        # Send status update with both API-reported and locally-counted tokens
        token_count = self._token_counter.session_count
...
class FlowRunner:
    ...
    async def run(self, soul: AxeSoul, args: str) -> None:
        if args.strip():
