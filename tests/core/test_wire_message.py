...
from axe_cli.wire.file import WireMessageRecord
from axe_cli.wire.serde import deserialize_wire_message, serialize_wire_message
...
async def test_wire_message_serde():
    ...
    assert serialize_wire_message(msg) == snapshot(
        {
            "type": "StatusUpdate",
            "payload": {"context_usage": 0.5, "token_usage": None, "message_id": None, "session_input_tokens": None, "session_output_tokens": None, "session_total_tokens": None},
        }
    )
    _test_serde(msg)
...
def test_wire_message_type_alias():
    import axe_cli.wire.types

    module = axe_cli.wire.types
    wire_message_types = {
        obj
