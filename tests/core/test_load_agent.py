...
from axe_cli.config import Config
from axe_cli.exception import InvalidToolError
from axe_cli.session import Session
from axe_cli.soul.agent import BuiltinSystemPromptArgs, Runtime, _load_system_prompt, load_agent
...
def test_load_system_prompt(system_prompt_file: Path, builtin_args: BuiltinSystemPromptArgs):
    ...
    assert builtin_args.AXE_NOW in prompt
    assert "test_value" in prompt


def test_load_tools_valid(runtime: Runtime):
    """Test loading valid tools."""
    tool_paths = ["axe_cli.tools.think:Think", "axe_cli.tools.shell:Shell"]
    toolset = AxeToolset()
    toolset.load_tools(
        tool_paths,
        {
...
def test_load_tools_invalid(runtime: Runtime):
    ...
    tool_paths = ["axe_cli.tools.nonexistent:Tool", "axe_cli.tools.think:Think"]
    toolset = AxeToolset()
    try:
        toolset.load_tools(
            tool_paths,
        ...
        assert "axe_cli.tools.nonexistent:Tool" in str(e)
...
agent:
  ...
  tools: ["axe_cli.tools.nonexistent:Tool"]
""")
...
def system_prompt_file() -> Generator[Path, Any, Any]:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        system_md = tmpdir / "system.md"
        system_md.write_text("Test system prompt with ${AXE_NOW} and ${CUSTOM_ARG}")
