...
from kosong.chat_provider.axe import Axe
from pydantic import SecretStr

from axe_cli.config import LLMModel, LLMProvider
from axe_cli.llm import augment_provider_with_env_vars, create_llm
...
def test_augment_provider_with_env_vars_kimi(monkeypatch):
    ...
    monkeypatch.setenv("KIMI_MODEL_NAME", "axe-env-model")
...
def test_create_llm_kimi_model_parameters(monkeypatch):
    ...
    assert isinstance(llm.chat_provider, Axe)
...
def test_create_llm_requires_base_url_for_kimi():
    provider = LLMProvider(type="axe", base_url="", api_key=SecretStr("test-key"))
    model = LLMModel(provider="axe", model="axe-base", max_context_size=4096)
