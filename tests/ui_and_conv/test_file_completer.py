...
from axe_cli.ui.shell.prompt import LocalFileMentionCompleter
...
def test_completed_file_short_circuits_completions(tmp_path: Path):
    ...
    nested_dir = tmp_path / "src" / "axe_cli" / "agents"
...
def test_basename_prefix_is_ranked_first(tmp_path: Path):
    ...
    (tmp_path / "src" / "axe_cli" / "tools" / "web").mkdir(parents=True)
    (tmp_path / "src" / "axe_cli" / "tools" / "file").mkdir(parents=True)

    fetch_py = tmp_path / "src" / "axe_cli" / "tools" / "web" / "fetch.py"
    fetch_py.write_text("# fetch\n")
    patch_py = tmp_path / "src" / "axe_cli" / "tools" / "file" / "patch.py"
    ...
    assert texts == snapshot(
        [
            "src/axe_cli/tools/web/fetch.py",
            "src/axe_cli/tools/file/patch.py",
        ]
    )
