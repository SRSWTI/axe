...
if [[ $SENSORS_STATUS -eq 0 && $CLIENT_STATUS -eq 0 ]]; then
    ...
    echo -e "  • Eject the disk image when done"
    echo -e "\n${YELLOW}Step 2 — Install BodegaOS Client (optional UI):${NC}"
    ...
    echo -e "    independently and does not require the Client to be installed."
    echo -e "  • ${GREEN}BodegaOS Client${NC} is optional — it's a UI for interacting with Sensors."
    echo -e "\n${BLUE}Getting Started with the Client:${NC}"
