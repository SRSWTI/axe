...
from axe_cli.config import LLMModel, LLMProvider, get_default_config
...
from axe_cli.wire.types import ContentPart, ToolReturnValue


class AventadorSoul(AxeSoul):
    @staticmethod
    async def create(
        llm: LLM | None,
        system_prompt: str,
        ...
        return AventadorSoul(agent, context=context)
...
async def main():
    ...
    soul = await AventadorSoul.create(
        llm=create_llm(
            LLMProvider(
                type="axe",
                base_url=os.getenv("CLOSEDAI_BASE_URL") or "https://api.closedai.com/v1",
                api_key=SecretStr(os.getenv("CLOSEDAI_API_KEY") or ""),
            ),
            LLMModel(
                provider="closedai",
        ...
        system_prompt="You are Aventador, an AI assistant that helps users with various tasks.",
        toolset=toolset,
    )
    ui = Shell(soul)
    await ui.run()
