"""
axe-psql: AI-assisted PostgreSQL interactive terminal.
...
from axe_cli.llm import LLM, create_llm
from axe_cli.session import Session
from axe_cli.soul import LLMNotSet, LLMNotSupported, MaxStepsReached, RunCancelled, run_soul
...
from axe_cli.wire.types import StatusUpdate
...
async def create_psql_soul(llm: LLM, conninfo: str) -> AxeSoul:
    """Create a AxeSoul configured for PostgreSQL with ExecuteSql tool
    and standard axe-cli tools."""
    ...
    cast(AxeToolset, agent.toolset).add(ExecuteSql(conninfo))

    context = Context(session.context_file)
    return AxeSoul(agent, context=context)
...
class PsqlShell:
    """Main TUI orchestrator for axe-psql."""

    PROMPT_SYMBOL_AI = "🪓"
    PROMPT_SYMBOL_PSQL = "$"

    def __init__(self, soul: AxeSoul, psql_process: PsqlProcess):
        self.soul = soul
    ...
    def _create_prompt_session(self) -> PromptSession[str]:
        ...
        def get_prompt() -> FormattedText:
            ...
            return FormattedText([("bold fg:blue", f"axe-psql{symbol} ")])
    ...
    async def _run_ai_mode(self) -> None:
        ...
        # Run soul with visualize (same as axe-cli shell)
        ...
        try:
            ...
            console.print("[red]LLM not set, run `axe /setup` to configure[/red]")
...
def main(
    ...
    """
    Start axe-psql: AI-assisted PostgreSQL interactive terminal.

    Usage is compatible with psql:
      axe-psql mydb
      axe-psql mydb postgres
      axe-psql -h localhost -U postgres -d mydb
      axe-psql --conninfo postgresql://user:pass@host/db
    """
...
async def _run_async(
    ...
    """Async entry point."""
    from axe_cli.config import load_config
    from axe_cli.llm import augment_provider_with_env_vars
    ...
    if not model:
        model = LLMModel(provider="axe", model="", max_context_size=250_000)
    if not provider:
        provider = LLMProvider(type="axe", base_url="", api_key=SecretStr(""))

    # Override with environment variables
    env_overrides = augment_provider_with_env_vars(provider, model)

    if not provider.base_url or not model.model:
        console.print("[red]LLM not configured. Run `axe /setup` to configure.[/red]")
        return
